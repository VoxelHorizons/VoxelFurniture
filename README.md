...
Use `neighbor_facing` when a rule must distinguish the neighbour's orientation. Supported values are `SAME` (the default), `PERPENDICULAR`, `OPPOSITE`, and `ANY`. For example, `neighbor_facing: PERPENDICULAR` selects a corner only when the required adjacent furniture is turned 90 degrees relative to the current piece.
